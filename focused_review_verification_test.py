#!/usr/bin/env python3
"""
Focused Review Request Verification Test
Testing specific endpoints mentioned in the review request for final verification.
"""
...
class FocusedReviewTester:
    ...
    def test_critical_endpoints(self):
        ...
        if response and response.status_code == 200:
            ...
            area10_present = any(area.get('area_id') == 'area10' for area in areas)
            self.log_test(
                "GET /assessment/schema/tier-based",
                True,
        ...
        session_data = {"area_id": "area1", "tier_level": 1}
        response, response_time = self.make_authenticated_request('POST', '/assessment/tier-session', 'client', session_data)
        if response and response.status_code in [200, 201]:
            data = response.json()
            session_id = data.get('data', {}).get('session', {}).get('id') or data.get('session_id')
            self.log_test(
                "POST /assessment/tier-session",
                True,
        ...
        if response and response.status_code in [200, 201]:
            data = response.json()
            request_id = data.get('request_id') or data.get('data', {}).get('service_request_id')
        ...
        response, response_time = self.make_authenticated_request('GET', '/analytics/dashboard', 'navigator')
        if response and response.status_code == 200:
            self.log_test(
                "GET /analytics/dashboard",
                True,
        ...
        payment_data = {"service_request_id": "test-request-123", "amount": 1500}
        response, response_time = self.make_authenticated_request('POST', '/payments/checkout/session', 'client', payment_data)
        if response and response.status_code == 200:
            data = response.json()
            has_checkout_url = 'checkout_url' in data or 'url' in data
            self.log_test(
                "POST /payments/checkout/session",
                True,
                f"Checkout session created: {has_checkout_url}",
                response_time
            )
        else:
            error_msg = response.text if response else "No response"
    ...
    def generate_summary(self):
        ...
        print("\n" + "=" * 60)
        print("🎯 FOCUSED REVIEW VERIFICATION SUMMARY")
        print("=" * 60)
    ...
    def run_focused_tests(self):
        """Run focused review verification tests"""
        print("🎯 FOCUSED REVIEW REQUEST VERIFICATION STARTED")
        print("Testing all endpoints mentioned in review request for final verification")
        print("=" * 60)
