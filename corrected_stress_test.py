#!/usr/bin/env python3
"""
CORRECTED COMPREHENSIVE STRESS TESTING AND CRITICAL ISSUE RESOLUTION
Using correct API endpoint paths from the backend
"""
...
class CorrectedStressTester:
    ...
    def test_ai_resources_rate_limiting(self):
        ...
        # Test the correct AI resources endpoint
        for i in range(10):
            try:
                start_time = time.time()
                response = self.session.get(
                    ...
                    headers=headers,
                    params={"question": f"Test question {i+1}"}
                )
                duration = time.time() - start_time
                ...
                if response.status_code == 200:
                    ...
                    self.log_result(f"AI Resources Request {i+1}", True, 
                                  f"Validation error (expected for test data)", duration=duration)
                else:
    ...
    def test_notification_api_errors(self):
        ...
        for role in ["client", "provider", "navigator", "agency"]:
            ...
            # Test notification endpoints that actually exist
            notification_endpoints = [
                ("GET", "/notifications", None),
                ("POST", "/notifications/send", {"message": "test", "recipient_id": "test"}),
    ...
    def test_certificate_generation_errors(self):
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                self.log_result("Certificate Generation", True,
                              f"403 Forbidden - likely missing permissions (expected)", duration=duration)
            else:
                self.log_result("Certificate Generation", False,
                              f"Unexpected status: {response.status_code}", response.text, duration)
    ...
    def test_phase4_multitenant_features(self):
        ...
        # Test multi-tenant endpoints that actually exist
        ...
        for method, endpoint, data in multitenant_endpoints:
            try:
                ...
                if response.status_code in [200, 201, 422]:  # Implemented (422 is validation error)
                    ...
                    self.log_result(f"Phase 4 Feature {method} {endpoint}", False,
                                  f"Method not allowed (405) - endpoint exists but wrong method", duration=duration)
                else:
                    self.log_result(f"Phase 4 Feature {method} {endpoint}", False,
                                  f"Unexpected status: {response.status_code}", response.text, duration)
    ...
    def concurrent_user_test(self, user_count=5, duration_seconds=30):
        ...
        def simulate_user_session(user_id):
            ...
            try:
                ...
                # Simulate user activities with real endpoints
                end_time = time.time() + duration_seconds
                request_count = 0
                
                while time.time() < end_time:
                    # Random API calls to existing endpoints
                    endpoints = [
                        ("GET", "/auth/me"),
                        ("GET", "/assessment/schema"),
                        ("GET", "/knowledge-base/areas"),
                        ("GET", "/client/assessment-progress"),
    ...
    def test_complete_user_journey(self):
        ...
        # 1. Create assessment session (correct endpoint)
        try:
            start_time = time.time()
            assessment_response = self.session.post(
                f"{BASE_URL}/assessment/session",
                headers=client_headers
            )
            duration = time.time() - start_time
            
            if assessment_response.status_code == 200:
                session_data = assessment_response.json()
                session_id = session_data.get("session_id")
                self.log_result("E2E: Assessment Creation", True,
                              f"Session created: {session_id}", duration=duration)
                
                # 2. Submit assessment responses (correct endpoint)
                if session_id:
                    start_time = time.time()
                    response_data = {
                        "area_id": "area1",
                    ...
                    submit_response = self.session.post(
                        f"{BASE_URL}/assessment/session/{session_id}/response",
                        headers=client_headers,
                        json=response_data
                    )
                    duration = time.time() - start_time
        ...
        # 3. Create service request (correct endpoint)
        try:
            start_time = time.time()
            ...
            if service_request.status_code in [200, 201]:
                request_data = service_request.json()
    ...
    def run_comprehensive_stress_test(self):
        ...
        print("🚨 STARTING CORRECTED COMPREHENSIVE STRESS TESTING")
        print("=" * 80)
    ...
    def generate_comprehensive_report(self, total_duration):
        ...
        print("\n" + "=" * 80)
        print("📊 CORRECTED COMPREHENSIVE STRESS TEST REPORT")
        print("=" * 80)
