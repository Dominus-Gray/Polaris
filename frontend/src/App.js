...
function QuestionCard({ area, q, sessionId, saveAnswer, current }) {
  ...
  const matchHref = `/matching?area_id=${area.id}&desc=${encodeURIComponent(q.text)}`;
...
function AssessmentApp() {
  ...
  const saveAnswer = async (areaId,qId,value,evidence_ids)=>{ setAnswers(prev=>({...prev,[areaId]:{...(prev[areaId]||{}),[qId]:{ value, evidence_ids:evidence_ids||[] }}})); if(!sessionId) return; await axios.post(`${API}/assessment/answers/bulk`, { session_id: sessionId, answers:[{ area_id: areaId, question_id: qId, value, evidence_ids }]}); if(value===false){ toast.info('Need help?', { description: 'Choose a free resource or request a provider match below.' }); } };
