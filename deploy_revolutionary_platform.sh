...
for i in {1..10}; do
    if curl -f -s https://polar-docs-ai.preview.emergentagent.com/api/system/health >/dev/null 2>&1; then
...
for endpoint in "${AI_ENDPOINTS[@]}"; do
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "https://polar-docs-ai.preview.emergentagent.com/api$endpoint" || echo "000")
...
RESPONSE_TIME=$(curl -s -w "%{time_total}" -o /dev/null "https://polar-docs-ai.preview.emergentagent.com/api/system/health")
...
for header in "${SECURITY_HEADERS[@]}"; do
    if curl -s -I "https://polar-docs-ai.preview.emergentagent.com/api/system/health" | grep -i "$header" >/dev/null; then
...
for test in "${FEATURE_TESTS[@]}"; do
    ...
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "https://polar-docs-ai.preview.emergentagent.com$ENDPOINT" 2>/dev/null || echo "000")
...
echo -e "${PURPLE}===================${NC}"
echo -e "${GREEN}✅ Backend: https://polar-docs-ai.preview.emergentagent.com/api${NC}"
