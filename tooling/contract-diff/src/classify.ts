import { SchemaChange, ChangeClassification, Config } from './types';
import { loadConfig } from './loadSchemas';

export function classify(changes: SchemaChange[]): ChangeClassification {
...
function classifyChange(change: SchemaChange, config: Config): keyof ChangeClassification {
...
function isDocsOnlyChange(change: SchemaChange, config: Config): boolean {
  const lastSegment = change.path[change.path.length - 1];
  
  // Check if the field matches docs-only patterns
  return config.docsOnlyPatterns.some(pattern => 
    lastSegment.toLowerCase().includes(pattern.toLowerCase())
  );
}

function isRefactorChange(change: SchemaChange): boolean {
  const pathStr = change.path.join('.');
...
function isOpenAPIBreakingChange(change: SchemaChange, pathStr: string): boolean {
...
function isOptionalToRequiredChange(change: SchemaChange, pathStr: string): boolean {
  ...
  if (pathStr.endsWith('required') && Array.isArray(change.newValue) && Array.isArray(change.oldValue)) {
    const addedRequired = change.newValue.filter((item: any) => !change.oldValue.includes(item));
...
export function isDeprecatedRemoval(change: SchemaChange, config: Config): boolean {
